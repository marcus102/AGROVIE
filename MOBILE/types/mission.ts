...
export type MissionStatus = 'in_review' | 'online' | 'rejected' | 'completed' | 'removed';
export type AdvantageType = Array<'meal' | 'accommodation'| 'performance_bonus' | 'transport'| 'other'>;
...
export type MissionBenefit = 'housing' | 'meals' | 'transportation' | 'insurance';
...
export interface MissionCreatePayload
  extends Omit<Mission, 'id' | 'created_at' | 'updated_at' | 'published_at' | 'final_price' | 'status'> {
  status?: MissionStatus;
}

export interface MissionUpdatePayload extends Partial<Omit<Mission, 'id' | 'user_id' | 'created_at' | 'updated_at'>> {}